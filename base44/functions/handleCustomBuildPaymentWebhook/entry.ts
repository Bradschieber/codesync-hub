...
Deno.serve(async (req) => {
  try {
    ...
    if (paymentType === 'custom_build_deposit') {
      const isFirstCustomBuild = order.is_first_custom_build || false;
      const newStatus = isFirstCustomBuild ? 'deposit_paid_pending_admin_release' : 'build_authorized';
      const newPayoutStatus = isFirstCustomBuild ? 'held_first_custom_deposit' : 'awaiting_release';
      const tiStatus = isFirstCustomBuild ? 'pending_admin_approval' : 'ready_for_transfer';
    ...
    if (paymentType === 'custom_build_final') {
      ...
      // TransferInstruction for final payout — held until shipment verified
      await base44.asServiceRole.entities.TransferInstruction.create({
        ...
        status: 'created', // Will become ready_for_transfer after shipment verified
      });
