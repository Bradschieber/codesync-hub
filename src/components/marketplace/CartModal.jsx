      ...
      <div className="relative bg-white w-full sm:w-96 h-full sm:h-auto sm:max-h-[85vh] sm:rounded-l-2xl shadow-2xl flex flex-col">
        <div className="flex items-center justify-between p-5 border-b border-stone-200">
          <div className="flex items-center gap-2">
            ...
            <span className="bg-wine-100 text-wine-700 text-xs font-medium px-2 py-0.5 rounded-full">{items.length}</span>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <button onClick={onClose} className="mt-3 text-wine-600 hover:underline text-sm">Browse guitars</button>
            </div>
          ) : (
            items.map(item => (
              <div key={item.id} className="flex gap-3 p-3 bg-stone-50 rounded-xl">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-wine-700 font-bold mt-1">{formatCurrency(item.product_price)}</p>
                </div>
        ...
        {items.length > 0 && (
          <div className="p-4 border-t border-stone-200 space-y-3">
            ...
            <Link
              ...
              onClick={onClose}
              className="block w-full bg-wine-600 hover:bg-wine-500 text-white text-center font-semibold py-3 rounded-xl transition-colors"
            >
              Proceed to Checkout
            </Link>
          </div>
