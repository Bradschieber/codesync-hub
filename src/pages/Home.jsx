        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-0 min-h-[580px] items-center">
            ...
            <div className="py-20 lg:py-28 lg:pr-12">
              ...
              <h1 className="text-5xl sm:text-6xl font-bold text-gray-900 leading-[1.05] mb-6 tracking-tight">
                Handcrafted<br />
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <Link
                  ...
                  className="bg-indigo-900 hover:bg-indigo-800 text-white font-semibold px-8 py-4 rounded-lg text-sm transition-colors text-center"
                >
                  Browse Guitars
                </Link>
                <Link
                  to={createPageUrl("CustomBuilds")}
                  className="border-2 border-gray-200 hover:border-indigo-900 hover:text-indigo-900 text-gray-700 font-semibold px-8 py-4 rounded-lg text-sm transition-colors text-center"
                >
                  Commission a Build
                </Link>
              </div>
            ...
            <div className="hidden lg:block relative h-full min-h-[580px] bg-gray-100">
              <img
                src="https://images.unsplash.com/photo-1510915361894-db8b60106cb1?w=900&q=85"
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <Link
                to={createPageUrl("CustomBuilds")}
                className="inline-flex items-center gap-2 bg-white text-indigo-900 hover:bg-gray-100 font-bold px-8 py-4 rounded-lg text-sm transition-colors"
              >
      ...
      <section className="border-t border-gray-100 bg-gray-50 py-16">
        <div className="max-w-xl mx-auto px-4 text-center">
          ...
          {subscribed ? (
            ...
            <form onSubmit={handleSubscribe} className="flex max-w-sm mx-auto gap-0 overflow-hidden rounded-lg border border-gray-300 bg-white shadow-sm">
              <input
                type="email"
                value={email}
              ...
              <button
                type="submit"
                className="bg-indigo-900 hover:bg-indigo-800 text-white text-sm font-semibold px-6 py-3 transition-colors whitespace-nowrap"
              >
                Subscribe
              </button>
            </form>
...
function ProductCard({ product }) {
  return (
    <Link
      ...
      className="group bg-white border border-gray-200 rounded-xl overflow-hidden hover:border-indigo-200 hover:shadow-lg transition-all duration-200"
    >
      ...
      <div className="p-4">
        ...
        <p className="text-indigo-900 font-bold text-base">${product.price?.toLocaleString()}</p>
      </div>
    </Link>
  );
}
...
function BuilderCard({ builder }) {
  return (
    <Link
      ...
      className="group bg-white border border-gray-200 rounded-xl p-5 hover:border-indigo-200 hover:shadow-md transition-all flex gap-4 items-start"
    >
      {builder.avatar_url ? (
        <img src={builder.avatar_url} className="w-14 h-14 rounded-full object-cover flex-shrink-0 border border-gray-200" />
      ) : (
        <div className="w-14 h-14 rounded-full bg-indigo-900 flex items-center justify-center flex-shrink-0">
          <span className="text-white font-bold text-xl">{builder.business_name?.[0] || builder.display_name?.[0] || "B"}</span>
        </div>
      )}
